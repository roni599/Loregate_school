...
<template>
    <div class="container">
        ...
        <div class="poster container mb-4 text-center">
            <div class="row px-4">
                <div v-for="religion in religionlists" :key="religion.id" class="col-6 col-md-3 mb-3 ">
                    <router-link :to="`/home-magazinelistview/${religion.id}`" class="text-decoration-none">
                        <div class="new_education">
                            <img :src="`/backend/images/magazine/${religion.image}`" :alt="religion.name"
                                class="img-fluid" width="308" height="120" />
                        </div>
                        <p>{{ religion.heading }}</p>
                        <p class="description">{{ truncateDescription(religion.description) }} <br> <span
                                class="mt-1">{{
                                    formatToBanglaDate(religion.date) }}</span></p>
                    </router-link>
                </div>
            </div>
        </div>
...
export default {
    name: "EducationView",
    setup() {
        const religionlists = ref([]);
        const router = useRouter();
        const religionListData = async () => {
            try {
                const response = await axios.get(`/api/academy/religion`);
                if (response.data.data) {
                    religionlists.value = response.data.data;
                }
            } catch (error) {
                console.log(error)
            }
