...
<template>
  <div class="container">
    ...
    <div class="poster container mb-4 text-center">
      <div class="row px-4">
        <div v-for="education in educationslist" :key="education.id" class="col-6 col-md-3 mb-3 ">
          <router-link :to="`/home-magazinelistview/${education.id}`" class="text-decoration-none">
            <div class="new_education">
              <!-- <img src="../../../../public/frontend/images/profile1.png" width="100px" alt=""> -->
              <img :src="`/backend/images/magazine/${education.image}`" :alt="education.name"
              class="img-fluid" width="308" height="120" />
            </div>
            <p>{{ education.heading }}</p>
            <p class="description">{{ truncateDescription(education.description) }} <br> <span class="mt-1">{{
              formatToBanglaDate(education.date) }}</span></p>
          </router-link>
        </div>
      </div>
    </div>
...
export default {
  ...
  setup() {
    ...
    const educationListData = async () => {
      try {
        const response = await axios.get(`/api/academy/education`);
        if (response.data.data) {
          educationslist.value = response.data.data;
        }
      } catch (error) {
        console.log(error)
      }
