...
<template>
    <div class="container">
        ...
        <div class="poster container mb-4 text-center">
            <div class="row px-4">
                <div v-for="story in storylists" :key="story.id" class="col-6 col-md-3 mb-3 ">
                    <router-link :to="`/home-magazinelistview/${story.id}`" class="text-decoration-none">
                        <div class="new_education">
                            <img :src="`/backend/images/magazine/${story.image}`" :alt="story.name"
                            class="img-fluid" width="308" height="120" />
                        </div>
                        <p>{{ story.heading }}</p>
                        <p class="description">{{ truncateDescription(story.description) }} <br> <span class="mt-1">{{
                                formatToBanglaDate(story.date) }}</span></p>
                    </router-link>
                </div>
            </div>
        </div>
...
export default {
    name: "EducationView",
    setup() {
        const storylists = ref([]);
        const router = useRouter();

        const storyListData = async () => {
            try {
                const response = await axios.get(`/api/academy/story`);
                if (response.data.data) {
                    storylists.value = response.data.data;
                }
            } catch (error) {
                console.log(error)
            }
