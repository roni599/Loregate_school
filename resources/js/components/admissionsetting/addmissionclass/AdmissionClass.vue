        ...
        <div class="card mt-4 mb-3">
            <div class="card-header border-bottom-0 p-4">
                <router-link class="text-decoration-none text-info h5"
                    to="/admin-dashboard">Dashboard</router-link><span class="text-muted h5"> / Education-Board</span>
            </div>
        </div>
        <div class="row mb-3">
...
import { ref, reactive, shallowRef, markRaw, watch } from 'vue';
export default {
    name: "AdmissionClass",
    setup() {
        const form = reactive({
        ...
        watch(() => showFields.group, (newVal) => {
            if (!newVal) form.group = '';
        });
        watch(() => showFields.session, (newVal) => {
            if (!newVal) form.session = '';
        });
        watch(() => showFields.shift, (newVal) => {
            if (!newVal) form.shift = '';
        });
        watch(() => showFields.section, (newVal) => {
            if (!newVal) form.section = '';
        });
